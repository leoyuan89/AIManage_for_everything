"""AIChatRenderer - AI 聊天消息渲染器"""
import re
from core.theme_manager import ThemeManager


class AIChatRenderer:
    """AI 聊天消息渲染器，将 Markdown 转为 HTML"""
    ...
    def welcome_md(self, vault_type: str) -> str:
    ...
    def render_assistant_md(self, msg, msg_index: int, is_last: bool = False,
                            thinking_expanded: dict = None, last_elapsed: float = 0.0) -> str:
        ...
        if msg.thinking and msg.thinking.strip() and not self.thinking_is_redundant(msg.thinking, msg.content):
            expanded = (thinking_expanded or {}).get(msg_index, False)
            if expanded:
    ...
    def markdown_to_html(self, text: str) -> str:
        ...
        # 分隔线 ---
        text = re.sub(r'^---+$', rf'<hr style="border:none;border-top:1px solid {colors.border_default};margin:8px 0;">', text, flags=re.MULTILINE)
