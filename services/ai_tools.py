...
【输出示例】
{{
  "{main_category}>考试": [101, 102],
  "{main_category}>课程平台": [103, 104, 105],
  "{main_category}>学术工具": [106]
}}"""
        else:
...
输出："""
        ...
        for cat_name, id_list in data.items():
            ...
            for tid in id_list:
                if tid in seen_ids:
                    print(f"[SmartClassify] SKIP duplicate id={tid} in category '{cat_name}'")
                    continue
                item = item_map.get(tid)
                if item:
                    seen_ids.add(tid)
...
4. 子类名绝对不能和以下一级分类名重复：
{top_level_text}
5. 你必须根据每个网址的标题、网址等信息，尽可能细分到合理的二级子类。不允许因为"看起来比较杂"就把所有条目归到同一个分类下偷懒
6. 优先匹配【当前分类体系】中已有的路径
7. 如需新建子类，子类名应简洁明确（2-4个字），如 `考试`、`课程平台`、`学术工具`、`语言学习`
8. 分类名中禁止包含 `/`、`·` 两个符号
9. 层级分隔符 `>` 最多出现一次，禁止输出三级及以上路径（如 `A>B>C` 是非法的）
...
输出："""
        ...
        for cat_name, id_list in data.items():
            ...
            for tid in id_list:
                if tid in seen_ids:
                    print(f"[SmartClassifyUrls] SKIP duplicate id={tid} in category '{cat_name}'")
                    continue
                item = item_map.get(tid)
                if item:
                    seen_ids.add(tid)
