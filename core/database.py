...
class DatabaseManager:
        ...
        with self._lock:
            try:
                ...
                new_columns = {
                    ...
                    'is_favorite': "ALTER TABLE accounts ADD COLUMN is_favorite INTEGER DEFAULT 0"
                }
    ...
    def _create_password_history_table(self):
        with self._lock:
            self.cursor.execute("""
    ...
    def get_iterations(self) -> int:
        """获取 PBKDF2 迭代次数（默认 100000，向后兼容）"""
        with self._lock:
            val = self._get_vault_config('pbkdf2_iterations')
            return int(val) if val else 100000
    
    def set_iterations(self, iterations: int):
        """设置 PBKDF2 迭代次数"""
        with self._lock:
            self._set_vault_config('pbkdf2_iterations', str(iterations))

    def get_breach_results(self, vault_type: str = 'accounts') -> Optional[dict]:
        """读取上次泄露检测结果"""
        import json
        with self._lock:
    ...
    def save_breach_results(self, vault_type: str = 'accounts', breached_ids: list = None, total_checked: int = 0):
        """保存泄露检测结果"""
        import json
    ...
    def clear_breach_results(self, vault_type: str = 'accounts'):
        """清除泄露检测结果"""
        with self._lock:
